...
namespace Qrame.CoreFX.Cryptography
{
    public static class AES256Crypto
    {
        ...
        public static bool IsSecretKey = false;
        public static byte[] SecretKey = new byte[32];

        public static void RequestSecretKey(string hexaKey, string baseDirectory)
        {
            try
            {
                Buffer.BlockCopy(HexaToBytes(hexaKey), 0, SecretKey, 0, 32);
                KMSKeyFault = null;
                IsSecretKey = true;
            }
            catch (Exception exception)
            {
                KMSKeyFault = DateTime.Now;
                IsSecretKey = false;
            }
        }
