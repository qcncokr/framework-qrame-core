...
namespace Qrame.CoreFX.HttpClient
{
	public static class HttpRequestFactory
	{
		...
		public static async Task<HttpResponseMessage> PostFile(string requestUri, string filePath, string paramName) => await PostFile(requestUri, filePath, paramName, "");

		public static async Task<HttpResponseMessage> PostFile(string requestUri, string filePath, string paramName, string bearerToken)
		{
			var builder = new HttpRequestBuilder()
				.AddMethod(HttpMethod.Post)
				.AddRequestUri(requestUri)
				.AddContent(new FileContent(filePath, paramName))
				.AddBearerToken(bearerToken);
